...
export const getAllRents = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ commercialRents: commercialRents });
  } catch (error) {
...
export const getOneRent = async(req , res , next)=>{
  try {
    ...
    const Rent = await commercialRent.findById(id);
    if(Rent){
...
export const updateRent = async (req, res, next) => {
  try {
    ...
    const Rent = await commercialRent.findById(id);
    if (Rent) {
      const updatedRent = await commercialRent.findByIdAndUpdate(
        id,
        { $set: req.body },
        { new: true }
      );
...
export const deleteRent = async (req, res, next) => {
  try {
    ...
    const Rent = await commercialRent.findById(id);
    if (Rent) {
      await commercialRent.findByIdAndDelete(id);
...
export const getRentsOfUser = async (req, res, next) => {
  try {
    ...
    const Rents = await commercialRent
      .find({
